...
getTransactionsBlock id_sender (transition: transList) = getTransaction id_sender transition ++ getTransactionsBlock id_sender transList
...
getListTransactions id_sender ((Block prevHash minerHash nonce transCount transList): xs) = getTransactionsBlock id_sender transList ++ getListTransactions id_sender xs

-- | Get user balance from transaction list
getBalance :: SenderHash -> TransList -> Amount
getBalance _ [] = 0
getBalance id_sender ((Transaction senderHash recvHash amount _): xs) = amount' + getBalance id_sender xs
 where
    amount'
...
prettyTransactions id_sender ((Transaction senderHash recvHash amount sign) : xs) = result ++ (prettyTransactions id_sender xs)
  where
    result 
