...
getListTransactions id_sender ((Block _prevHash _minerHash _nonce _transCount transList): xs) =
...
getAmountMinedBlocks id_sender ((Block _ minerHash _ _ _): xs)
...
prettyTransactions id_sender ((Transaction senderHash recvHash amount time _) : xs) = 
